...
=== Using the library with `opentest4j`

Create a file `pcond.properties` whose content is as follows in order to configure the `pcond` to use `opentest4j` instead of JUnit4's exceptions.

.pcond.properties
----
exceptionComposerForAssertThat=com.github.dakusui.pcond.validator.ExceptionComposer$ForTestAssertion$Opentest4J
----

Place it at the top level of one of your classpath entries.
In maven, the directory `src/main/resources` is a good place for that.
...
You can see the violated condition and how it is violated in the exception message.
